...
> 5. **Fluxo de trabalho (branch + merge — não publicar cedo)**: cada iteração começa por **criar um branch novo** (nome à escolha do utilizador, ex.: `feat/<tema>`) a partir do `main` atualizado (`git pull` antes) e **enviá-lo de imediato para o GitHub** (`git push -u origin <branch>`) — enviar um branch **não publica o Pages** (só o `main` é que publica); todas as alterações/commits (script, styles, html, manual, changelog, progress) ficam nesse branch; **testar sempre localmente primeiro** (`node --check` + smoke-test quando a lógica de render muda) e só depois dar a iteração como pronta; **nunca publicar o GitHub Pages de imediato** — o deploy acontece **apenas quando o utilizador indicar**, fazendo **merge do branch no `main`** (`git checkout main` + `git merge --no-ff <branch>` + `git push origin main`), porque o Pages constrói automaticamente a partir do `main`. Após o merge: verificar o site ao vivo (HTTP 200 + versão) e **manter o branch** (local e remoto) — **só se apaga quando o utilizador pedir explicitamente**.
...
- **Regras aplicadas durante o desenvolvimento:**
  ...
  - **Fluxo por iteração (regra 5)**: `git pull origin main` → criar branch novo (nome **à escolha do utilizador**, `git checkout -b <nome>`) → **enviá-lo já para o GitHub** (`git push -u origin <nome>` — **não publica o Pages**, só o `main` publica) → alterar/comitar **no branch** → **testar localmente primeiro** (`node --check` e smoke-test, que é hermético/data fixa) → **não publicar o GitHub Pages**: só quando o utilizador indicar, `git checkout main; git merge --no-ff <branch>; git push origin main` (o Pages publica sozinho a partir de `main`) → verificar `https://rjduraocosta.github.io/solmeteo/` (HTTP 200 + versão) → **manter o branch (não apagar)** — o branch só é apagado quando o utilizador o pedir explicitamente.
